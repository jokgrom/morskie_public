<?php
    ...
    list  ($productId, $boolError, $textError)=$CleanFormProduct->id($_COOKIE['lastResidenceId']);
    if($boolError){array_push($statusError, $textError);}
    if(!count($statusError)){
        ...
        if($boolError){
            ...
            $countPhotoResidence=$Checks->countPhotoResidence($personId, $productId);
            if($countPhotoResidence>=15){
                array_push($statusError, "Превышен лимит загружаемых фотографий $countPhotoResidence");
            }
        }
    }
    ...
    if (isset($_FILES['images']) AND !count($statusError)) {
        ...
        foreach ($files as $file) {
            if (!empty($file['error']) || empty($file['tmp_name'])) {
                ...
                if ($image[1] > $limitHeightMax) {$statusError.= 'Высота изображения не должна превышать 12500 пикселей. '; continue;}
                if ($image[1] < $limitHeightMin) {$statusError.= 'Высота изображения не должна быть меньше 200 пикселей. '; continue;}
                if ($image[0] > $limitWidthMax) {$statusError.= 'Ширина изображения не должна превышать 12500 пикселей. '; continue;}
                ...
                if (imagejpeg($source, $dir2 . $namePhoto .'.jpg', 75)) {
                    ...
                    //Добавим в бд
                    $countPhoto++;
                    $MyResidence->addPhoto($personId, $productId, $namePhoto, $sizeWidth, $sizeHeight, $sizeBytes, $path,$countPhoto);

                    //если превышен лимит то выйдем из цикла добавления
                    if($countPhotoResidence+$countPhoto>=15){ break; }
                    continue;
                }
            }
        }
        if($countPhoto>=1){
            ...
            exit('<div class="modal"><p>Загружено фотографий: '.$countPhoto.'!</p></div>
                <script  type="text/javascript">
                    setTimeout(function(){
