<?php
    ...
    list  ($productId, $boolError, $textError)=$CleanFormProduct->id($_COOKIE['lastEntertainmentId']);
    ...
    if(!count($statusError)){
        ...
        if($boolError){
            ...
            $countPhotoEntertainment=$Checks->countPhotoEntertainment($personId, $productId);
            if($countPhotoEntertainment>=15){
                array_push($statusError, "Превышен лимит загружаемых фотографий $countPhotoEntertainment");
            }
        }
    }
    ...
    if (isset($_FILES['images']) AND !count($statusError)) {
        $dir1 = $_SERVER['DOCUMENT_ROOT']."/photoEntertainment/".$personId;
        $dir2 = $_SERVER['DOCUMENT_ROOT']."/photoEntertainment/".$personId."/".$productId."/";
        ...
        foreach ($files as $file) {
            if (!empty($file['error']) || empty($file['tmp_name'])) {
                ...
                if (imagejpeg($image_p, $dir2 . $namePhoto .'.jpg', 79)) {
                    ...
                    $namePhoto.='.jpg';
                    $path="/photoEntertainment/".$personId."/".$productId."/";
                    //Добавим в бд
                    $countPhoto++;
                    $MyEntertainment->addPhoto($personId, $productId, $namePhoto, $sizeWidth, $sizeHeight, $sizeBytes, $path,$countPhoto);

                    //если превышен лимит то выйдем из цикла добавления
                    if($countPhotoEntertainment+$countPhoto>=15){ break; }
                    continue;
                }
            }
        }
